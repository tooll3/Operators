    ...
    public class GetListItemAttribute : Instance<GetListItemAttribute>
    {
        ...
        private void Update(EvaluationContext context)
        {
            ...
            if (needToUpdateIndex)
            {
                ...
                Log.Warning($"There is not Float field for index {requestedFieldIndex} or name '{requestedFieldName}' in type {list.Type.Name}", SymbolChildId);
            }
            else if (_field?.GetValue(item) is float f)
            {
                Result.Value = f;
            }
        }
